...
final class PHPNamespaceTest extends TestCase
{
	...
	public function testRejectsAnonymousFunctionAsDirectChild(): void
	{
		$ns = new PHPNamespace('App');
		$fn = new PHPFunction();
		$this->assertThrows(
			'anonymous function in namespace throws',
			InvalidArgumentException::class,
			static fn () => $ns->addChild($fn)
		);
	}

	public function testAcceptsNamedFunctionAsDirectChild(): void
	{
		$ns = new PHPNamespace('App');
		$fn = new PHPFunction('helper');
		$ns->addChild($fn);
		$this->assertEq('named function accepted', 1, \count($ns->getChildren()));
	}
}
