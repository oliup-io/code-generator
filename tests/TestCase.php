...
abstract class TestCase extends PHPUnitTestCase
{
    ...
    {
        $this->assertStringContainsString($needle, $haystack, $description);
    }
    ...
    {
        $this->assertStringNotContainsString($needle, $haystack, $description);
    }
    ...
    protected function assertThrows(string $description, string $exceptionClass, callable $fn): void
    {
        try {
            $fn();
            $this->fail($description . ' (no exception thrown)');
        } catch (AssertionFailedError $e) {
            throw $e;
        } catch (\Throwable $e) {
            if (!($e instanceof $exceptionClass)) {
                $this->fail($description . ' (wrong exception: ' . \get_class($e) . ')');
            }
    ...
    protected function assertSnapshot(string $snapshotFile, string $actual): void
    {
        ...
        $this->assertSame((string) \file_get_contents($snapshotFile), $actual, 'snapshot mismatch: ' . \basename($snapshotFile));
    }
}
